"""
step04a_build_meerkat_catalogue_and_multisurvey_figures.py
==================================
...
Inputs
------
    03_Outputs/step02a_meerkat_matched.vot — 188 cross-matched sources
    03_Outputs/step03b_meerkat_visual.vot — visually confirmed sources
                                     *** Optional: if not found, proceeds
                                         with cross-matched only ***
    03_Outputs/step03a_meerkat_outside.vot — outside MeerKAT region
    03_Outputs/step03a_meerkat_to_check.vot — inside region, not detected
    03_Outputs/step02b_askap_matched.vot — accepted ASKAP associations
...
Catalogues (03_Outputs/):
    step04a_meerkat_detected.vot — cross-matched + visually confirmed
    step04a_meerkat_nondetected.vot — inside region, not detected

Paper figures (04_Figures/) — NO TITLES on any figure:
    step04a_multisurvey_sky_map_detections.pdf — footprints and survey overlap
    step04a_multisurvey_sky_map_reid_classes.pdf — footprints and Reid classes
    step04a_multisurvey_detection_summary.pdf — survey totals + MeerKAT class bars
    step04a_meerkat_positional_offsets.pdf — Reid-versus-radio offsets
    step04a_meerkat_snr_distribution.pdf — S/N distribution
    step04a_meerkat_detection_breakdown.pdf — source-count breakdown
...
print("step04a: building MeerKAT catalogue")

# Load all catalogues
print("\n[1] Loading catalogues...")
matched = Table.read(os.path.join(OUTS, "step02a_meerkat_matched.vot"), format="votable")
outside = Table.read(os.path.join(OUTS, "step03a_meerkat_outside.vot"), format="votable")
to_check = Table.read(os.path.join(OUTS, "step03a_meerkat_to_check.vot"), format="votable")

visual_path = os.path.join(OUTS, "step03b_meerkat_visual.vot")
if os.path.exists(visual_path):
    visual = Table.read(visual_path, format="votable")
    print(f"    step03b_meerkat_visual.vot : {len(visual)} visually confirmed")
else:
    visual = None
    print(f"    step03b_meerkat_visual.vot: NOT FOUND (proceeding with cross-matched only)")

print(f"    step02a_meerkat_matched.vot : {len(matched)} cross-matched")
print(f"    step03a_meerkat_outside.vot : {len(outside)} outside region")
print(f"    step03a_meerkat_to_check.vot: {len(to_check)} to check")

askap_matched = Table.read(ASKAP_MATCHED_FILE, format="votable")
print(f"    step02b_askap_matched.vot   : {len(askap_matched)} accepted")

reid_path = os.path.join(OUTS, "step01a_reid_master.vot")
...
detected.write(os.path.join(OUTS, "step04a_meerkat_detected.vot"), format="votable", overwrite=True)
nondetected.write(os.path.join(OUTS, "step04a_meerkat_nondetected.vot"), format="votable", overwrite=True)

print(f"    step04a_meerkat_detected.vot -> {n_det} rows "
      f"({n_xmatch} cross-matched + {n_visual} visual)")
print(f"    step04a_meerkat_nondetected.vot -> {n_nondet} rows")
...
# step04a_multisurvey_sky_map_detections.pdf
...
leg_det = ax.legend(
    handles=detection_legend, title="Reid-source detections",
...
info = ("FITS masks reprojected to a common ICRS SIN grid\n"
        "MeerKAT detections: 188 catalogue + 32 visual")
...
plt.savefig(os.path.join(FIGS, "step04a_multisurvey_sky_map_detections.pdf"),
            format="pdf", bbox_inches="tight", facecolor="white")
plt.close()
print("    step04a_multisurvey_sky_map_detections.pdf")

# step04a_multisurvey_sky_map_reid_classes.pdf
...
class_order = ["True", "Known", "Likely", "Possible", "Unknown"]
...
leg_cls = ax.legend(
    handles=class_legend, title="Reid optical classification",
    fontsize=8.8, title_fontsize=9.2, framealpha=0.96,
    loc="upper left", frameon=True, borderpad=0.65,
)
...
fig.subplots_adjust(left=0.11, right=0.985, bottom=0.09, top=0.985)
plt.savefig(
    os.path.join(FIGS, "step04a_multisurvey_sky_map_reid_classes.pdf"),
    format="pdf", bbox_inches="tight", facecolor="white",
)
plt.close()
print("    step04a_multisurvey_sky_map_reid_classes.pdf")

# step04a_meerkat_positional_offsets.pdf: cross-matched and visual combined
...
ax_mn.add_artist(at)
plt.savefig(os.path.join(FIGS, "step04a_meerkat_positional_offsets.pdf"),
            format="pdf", bbox_inches="tight", facecolor="white")
plt.close(); plt.style.use("default")
print(f"    step04a_meerkat_positional_offsets.pdf (N={len(x_all)}: {n_xmatch} XM + {len(x_v)} visual)")

# step04a_meerkat_snr_distribution.pdf: cross-matched and visual combined
...
plt.tight_layout()
plt.savefig(os.path.join(FIGS, "step04a_meerkat_snr_distribution.pdf"),
            format="pdf", bbox_inches="tight", facecolor="white")
plt.close()
print(f"    step04a_meerkat_snr_distribution.pdf (N={len(snr_vals)}: {len(snr_xm)} XM + {len(snr_vi)} visual)")

# step04a_meerkat_detection_breakdown.pdf: donut, counts only
n_inside_det   = n_det
...
plt.tight_layout()
plt.savefig(os.path.join(FIGS, "step04a_meerkat_detection_breakdown.pdf"),
            format="pdf", bbox_inches="tight", facecolor="white")
plt.close()
print(f"    step04a_meerkat_detection_breakdown.pdf")

# step04a_multisurvey_detection_summary.pdf
...
ax.text(0.0, 1.055, "MeerKAT detection status by Reid optical class",
...
plt.savefig(os.path.join(FIGS, "step04a_multisurvey_detection_summary.pdf"),
            format="pdf", bbox_inches="tight", facecolor="white")
plt.close()
print(f"    step04a_multisurvey_detection_summary.pdf")
...
print(f"    step04a_meerkat_detected.vot    ({n_det} rows)")
print(f"    step04a_meerkat_nondetected.vot ({n_nondet} rows)")
print(f"\n  Paper figures (04_Figures/):")
print(f"    step04a_multisurvey_sky_map_detections.pdf")
print(f"    step04a_multisurvey_sky_map_reid_classes.pdf")
print(f"    step04a_multisurvey_detection_summary.pdf")
print(f"    step04a_meerkat_positional_offsets.pdf")
print(f"    step04a_meerkat_snr_distribution.pdf")
print(f"    step04a_meerkat_detection_breakdown.pdf")
