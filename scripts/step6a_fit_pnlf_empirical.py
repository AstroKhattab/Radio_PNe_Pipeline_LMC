"""
step6a_fit_pnlf_empirical.py — Step 07a: catalogue + model fits
================================================================
...
# two fits are done by identical code and any difference between them is a
# difference in the sample rather than in the treatment.
#     python3 step6a_fit_pnlf_empirical.py            full sample
#     python3 step6a_fit_pnlf_empirical.py --sample high
SAMPLE = "full"
if "--sample" in sys.argv:
    SAMPLE = sys.argv[sys.argv.index("--sample") + 1]
if SAMPLE not in ("full", "high"):
    raise SystemExit("--sample must be 'full' or 'high'")
...
finite_flux=np.isfinite(rfj)&(rfj>0)
# Apply the flux ceiling at source level rather than by dropping whole bins,
# so the objects concerned leave the sample outright instead of leaving a hole
# in the middle of the binned distribution.
_lum_tmp=np.full(len(det),np.nan)
_lum_tmp[finite_flux]=4*np.pi*D_CM**2*rfj[finite_flux]*1e-23
_M_tmp=np.full(len(det),np.nan)
_M_tmp[finite_flux]=-2.5*(np.log10(_lum_tmp[finite_flux])-np.log10(L_REF))
above_ceiling=finite_flux&(_M_tmp<M_CEIL)
# Sources above the ceiling stay in the catalogue and stay on the plot, drawn
# as open symbols; they are simply not fitted.  Dropping them outright would
# hide from the reader that they exist, and their position relative to the
# cutoff is part of the argument for the ceiling in the first place.
valid=has_meerkat&finite_flux
pnlf_fitted=valid&~above_ceiling
print(f"    above the {M_CEIL:.2f} mag flux ceiling, shown but not fitted: {int((has_meerkat&above_ceiling).sum())}")
...
det.add_column(Column(pnlf_fitted.astype("i2"), name="pnlf_included",
                      description="1 if the source enters the PNLF fit"))
det.add_column(Column(valid.astype("i2"), name="pnlf_plotted",
                      description="1 if the source is shown on the PNLF plot"))
...
pnlf_reason[has_meerkat&~finite_flux]="excluded_invalid_MeerKAT_flux"
pnlf_reason[has_meerkat&above_ceiling]="excluded_above_flux_ceiling"
...
# Two exclusions, both applied at bin level so the points remain visible:
# brighter than the physical ceiling, and fainter than the completeness limit.
too_bright=mx<M_CEIL; incompl=mx>M_LIM; fit_m=~too_bright&~incompl
...
m_peak=mx_f[np.argmax(my_f)]; M_fit_bright=mx_f.min()
Ms_max=M_fit_bright-0.05; Ms_min=M_fit_bright-5.0
print(f"\n[2] Bins in fit: {fit_m.sum()}  excluded: {(~fit_m).sum()}"
      f"  (above ceiling: {too_bright.sum()}, incomplete: {incompl.sum()})")
...
with open(_pcsv, "w") as _fc, open(_ptab, "w") as _ft:
    ...
    _ft.write("%% written by %s --sample %s -- do not edit by hand\n" % (os.path.basename(__file__), SAMPLE))
    _ft.write("\\begin{tabular}{clrrrrl}\n\\toprule\n")
    _ft.write("Rank & Model & $k$ & AIC & $\\Delta$AIC & $\\chi^{2}_{\\nu}$ & Fitted parameters \\\\\n\\midrule\n")
    for _i, (_nm, _r) in enumerate(ranked):
        _st = _r["s"]; _pp = list(_r["popt"])
        _pstr = ", ".join("%.4g" % v for v in _pp)
        _fc.write('%d,"%s",%d,%d,%.4f,%.2f,%.2f,%.2f,%.3f,"%s"\n' %
                  (_i+1, _nm, _st["k"], _st["dof"], _st["r2"], _st["aic"],
                   _st["aic"]-_aicmin, _st["bic"], _st["redchi"], _pstr))
        _nmtex = _nm.replace("_", "\\_").replace("Student-t", "Student-$t$")
        _ft.write("%d & %s & %d & %.2f & %.2f & %.3f & %s \\\\\n" %
                  (_i+1, _nmtex, _st["k"], _st["aic"],
                   _st["aic"]-_aicmin, _st["redchi"], _pstr))
    _ft.write("\\bottomrule\n\\end{tabular}\n")
print(f"    wrote {os.path.basename(_ptab)} and {os.path.basename(_pcsv)}")
...
def plot_data(ax):
    ...
    ax.errorbar(mx_o,my_o,yerr=mye_o,fmt="o",capsize=4,markersize=8,mfc="white",
                ...
                linestyle="none",zorder=20,label=f"Not fitted  (M < {M_CEIL} or M > {M_LIM})")
...
with PdfPages(OUT_FIG) as pdf:
    ...
    ax_d.errorbar(mx_o, my_o, yerr=mye_o, fmt="o", capsize=4, markersize=8,
                  ...
                  label=f"Not fitted  (M < {M_CEIL} or M > {M_LIM})")
    ax_d.axvline(M_LIM, color="#aaa", ls="--", lw=1.2)
    ax_d.text(M_LIM + 0.07, 0.95, f"Mlim={M_LIM}\n(5sig~{S_MLIM_UJY:.0f} uJy)",
              fontsize=8.5, color="#666", ha="left", va="top",
              transform=ax_d.get_xaxis_transform(),
              bbox=dict(boxstyle="round,pad=0.2", fc="white", alpha=0.8, ec="none"))
    # mark the physical bright limit
    ax_d.axvline(M_CEIL, color="#C62828", lw=1.4, zorder=1,
                 label=f"Flux ceiling (M = {M_CEIL}, 2.2 mJy)")
    ...
    for ci,(nm,r) in enumerate(ranked):
        ...
        ax.plot(m_fine,np.where(yf>0.3,yf,np.nan),"-",color=col,lw=lw,alpha=0.9,
                label=f"{nm}  (AIC={r['s']['aic']:.1f}, $\\chi^2_\\nu$={r['s']['redchi']:.2f})")
    fmt_ax(ax)
...
print("\nstep6a complete: model comparison summary")
