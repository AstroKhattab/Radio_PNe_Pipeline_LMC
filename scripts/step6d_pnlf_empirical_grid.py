...
Outputs
    05_Figures/step6d_pnlf_empirical_grid_10up.pdf   one 5x2 page, used in the paper
    05_Figures/step6d_pnlf_empirical_grid_4up.pdf    three 2x2 pages
...
SAMPLE="full"
if "--sample" in sys.argv: SAMPLE=sys.argv[sys.argv.index("--sample")+1]
...
m_peak=mx_f[np.argmax(my_f)]; Ms_max=mx_f.min()-0.05; Ms_min=mx_f.min()-5.0
...
def panel(ax,nm,r,rank,small=False):
    ...
    ax.text(0.03,0.94,f"AIC {r['aic']:.1f}   $\\chi^2_\\nu$ {r['redchi']:.2f}",transform=ax.transAxes,
...
fig.suptitle(f"Radio PNLF — empirical model fits, ranked by AIC{_SLBL}",fontsize=11.5,y=0.99)
fig.tight_layout(rect=[0,0,1,0.96])
fig.savefig(os.path.join(FIG,f"{STEP}_pnlf_empirical_grid_10up.pdf")); plt.close(fig)

# --- 4-up, 3 pages ---
with PdfPages(os.path.join(FIG,f"{STEP}_pnlf_empirical_grid_4up.pdf")) as pdf:
    for pg in range(3):
        ...
        fig.suptitle(f"Radio PNLF — empirical model fits{_SLBL} ({pg+1}/3)",fontsize=11)
