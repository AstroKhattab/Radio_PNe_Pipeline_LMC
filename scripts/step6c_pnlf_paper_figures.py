...
are repeated here from step6a_pnlf.vot; nothing upstream is modified.

Outputs (05_Figures/):
    step6c_pnlf_model_overlay.pdf — top empirical fits + data
    step6c_pnlf_ciardullo_comparison.pdf — three Ciardullo variants

Run after Step 07a has produced step6a_pnlf.vot:
    cd ~/Desktop/PN\ LMC\ Paper
    python 02_Scripts/step6c_make_pnlf_paper_figures.py
...
print("\n[4] Generating step6c_pnlf_model_overlay.pdf ...")
...
STYLES_EMP = ["-", "--", "-.", ":"]
# The models are ranked by AIC, so the legend quotes AIC (as a difference from
# the best model, which is what an AIC comparison actually means) together with
# the reduced chi-square.  R-squared was shown here previously and is a poor
# summary for models with different numbers of parameters.
_aic_best = min([r["aic"] for r in top_empirical] +
                ([top_ciar[0]["aic"]] if top_ciar else []))
...
for i, r in enumerate(top_empirical):
    ...
    ax.plot(mplot, np.where(yf > 0.3, yf, np.nan), STYLES_EMP[i % len(STYLES_EMP)],
            color=COLORS_EMP[i], lw=2.0, alpha=0.85,
            label=(f"{r['name']}  ($\\Delta$AIC$\\,{r['aic']-_aic_best:.1f}$, "
                   f"$\\chi^2_\\nu={r['redchi']:.2f}$)"))
...
if top_ciar:
    ...
    ax.plot(mplot, np.where(yf > 0.3, yf, np.nan), "--",
            ...
            label=(f"Ciardullo canonical  ($\\Delta$AIC$\\,{rc['aic']-_aic_best:.1f}$, "
                   f"$\\chi^2_\\nu={rc['redchi']:.2f}$)"))
...
print("\n[5] Generating step6c_pnlf_ciardullo_comparison.pdf ...")
...
print(f"  -> step6c_pnlf_model_overlay.pdf")
print(f"  -> step6c_pnlf_ciardullo_comparison.pdf")
