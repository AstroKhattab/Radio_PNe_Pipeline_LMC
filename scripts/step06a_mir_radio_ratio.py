"""
step06a_mir_radio_ratio.py
===========================
Step 6a — MIR/radio diagnostic at three measured radio frequencies.
...
This step therefore reports the ratio separately using:
    ...
    2. MeerKAT channel 4 at 996.646 MHz (the closest measured channel to
       the Cohen et al. common ~1-GHz convention);
    3. MeerKAT broadband integrated flux at 1295 MHz.

The adopted near-1-GHz diagnostic uses the 996.646-MHz MeerKAT channel when
available, otherwise ASKAP 888 MHz, otherwise MeerKAT 1295 MHz.  No radio
frequency scaling is hidden in these columns: every ratio uses the measured
flux at the frequency stated in its name.

Inputs
------
    03_Outputs/step05a_spectral_index.vot

Outputs
-------
    03_Outputs/step06a_mir_radio_ratio.vot
    04_Figures/step06a_mir_radio_ratio.pdf
...
INPUT_FILE = os.path.join(OUTS, "step05a_spectral_index.vot")
OUT_VOT = os.path.join(OUTS, "step06a_mir_radio_ratio.vot")
FIG_RATIO = os.path.join(FIGS, "step06a_mir_radio_ratio.pdf")
...
print("step06a: mid-infrared to radio ratio diagnostic")
...
mkt_ch4_flux_jy = numeric(detected, "mkt_ch4_996p646_flux_Jy")
mkt_ch4_error_jy = numeric(detected, "mkt_ch4_996p646_err_Jy")
...
ratio_ch4, ratio_ch4_error = ratio_with_error(
    flux_8um_jy, flux_8um_error_jy, mkt_ch4_flux_jy, mkt_ch4_error_jy
)
...
# convention: MeerKAT 996.646 MHz, then ASKAP 888 MHz, then 1295 MHz.
...
for ratio, error, frequency, label in [
    (ratio_ch4, ratio_ch4_error, 996.646, "MeerKAT_ch4"),
...
new_columns = {
    ...
    "cohen_ratio_mkt_996p646": ratio_ch4.astype("f4"),
    "cohen_ratio_mkt_996p646_err": ratio_ch4_error.astype("f4"),
...
print("\n[5] Generating the three-frequency comparison figure...")
fig, axes = plt.subplots(1, 3, figsize=(15, 4.8), sharey=True)
panels = [
    (ratio_askap, "ASKAP 888 MHz", "#E67E22"),
    # (ratio_ch4, "MeerKAT 996.646 MHz", "#2166AC"),
    # changed 2026-08-30: panel title rounded to the frequency the paper quotes
    (ratio_ch4, "MeerKAT 997 MHz", "#2166AC"),
...
fig.suptitle("Cohen MIR/radio diagnostic at the three measured radio frequencies")
plt.tight_layout()
